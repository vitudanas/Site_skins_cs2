...
for(const file of new Set(files)){if(forbidden.some(r=>r.test(file))){issues.push(file+': arquivo privado');continue;}if(file.endsWith('.png'))continue;const body=fs.readFileSync(path.join(root,file),'utf8');if(secrets.some(r=>r.test(body)))issues.push(file+': possível segredo');if(/[A-Z]:[\\/](?:Users|Documents and Settings)[\\/]/i.test(body)||/\/Users\/[^/]+\//.test(body))issues.push(file+': caminho pessoal absoluto');if(/appg(?:prj|dep|ver)_/.test(body))issues.push(file+': identificador interno');}
